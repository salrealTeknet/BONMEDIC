...
def near_expiry_products(request):
        today = datetime.today().date()
        near_expiry_date = today + timedelta(days=30)
        near_expiry_stocks = Stock.objects.filter(expiry_date__lte=near_expiry_date, expiry_date__gte=today,
                                                  is_deleted=False)

        context = {
            'near_expiry_stocks': near_expiry_stocks
        }
        return render(request, 'near_expiry_products.html', context)

def export_near_expiry_to_csv(request):
    today = datetime.today().date()
    near_expiry_date = today + timedelta(days=30)
    near_expiry_stocks = Stock.objects.filter(expiry_date__lte=near_expiry_date, expiry_date__gte=today,
                                                  is_deleted=False)
    ...
    response['Content-Disposition'] = 'attachment; filename="near_expiry_products.csv"'

    writer = csv.writer(response)
    writer.writerow(['ID', 'Name', 'Quantity', 'Expiry Date'])

    for stock in near_expiry_stocks:
            writer.writerow([stock.id, stock.name, stock.quantity, stock.expiry_date])
            return response
