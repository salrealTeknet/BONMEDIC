...
def export_to_csv(products):
    ...
    response['Content-Disposition'] = 'attachment; filename="about_to_expire_products.csv"'

    writer = csv.writer(response)
    writer.writerow(['Product Name', 'Expiry Date'])

    for product in products:
        writer.writerow([product.name, product.expiry_date])
...
def about_to_expire_products(request):
    ...
    warning_date = today + timedelta(days=30)  # Define 'about to expire' as within the next 30 days
    products = Supplier.objects.filter(expiry_date__lte=warning_date, expiry_date__gte=today)

    if 'export' in request.GET:
        return export_to_csv(products)

    return render(request, 'near_expiry_products.html', {'products': products})
